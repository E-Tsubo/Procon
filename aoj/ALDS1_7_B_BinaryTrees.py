...
def main():
	...
	for i in range(n):	
		...
		#print(i, tree[i].getData(), tree[i].getRightIdx(),tree[i].getRight() )
		
	#BinaryTreeの情報を出力
	for i in range(n):
		for j in range(n):
			if tree[j].getData() == i:
				print("node {}: parent = {}, sibling = {}, degree = {}, depth = {}, height = {}, {}".format(tree[j].getData(), tree[j].getParent(), calcSibling(tree, j), calcDegree(tree[j]), calcDepth(tree, j), calcHeight(tree, j), calcNodeType(tree[j]) ) )
	
		#print("node:", i, "parent =", tree[i].getParent(), ",sibling =", calcSibling(tree, i), end=" ")
		#print(", degree =", calcDegree(tree[i]), ", depth =", calcDepth(tree, i), end=" " )
		#print(calcHeight(tree, i), end=" ")
		#print(calcNodeType(tree[i]))
